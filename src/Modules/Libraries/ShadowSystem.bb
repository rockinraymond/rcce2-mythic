...
Global Caster_Radius_Fudge_Factor# = 1.5
...
; -------------------------------------------------------------------------------------------------------------------
Function Cast_Shadow(Caster, Resolution=512, Translucent=0)
		...
		If Light_To_Caster_Distance# <= ThisLight\Range#
			...
			If ThisCaster\RenderTexture = 1 
				...
				If Render_Texture Or Animating(ThisCaster\Entity)
					...
					CameraViewport Shadow_Cam, 0, 0, ThisCaster\Resolution+100, ThisCaster\Resolution+100
	
					; Render the shadow view.
					RenderWorld

					; Copy the new shadow texture from the screen buffer to the texture buffer.		
					CopyRect 50, 50, ThisCaster\Resolution, ThisCaster\Resolution, 0, 0, BackBuffer(), TextureBuffer(ThisSet\Texture)
