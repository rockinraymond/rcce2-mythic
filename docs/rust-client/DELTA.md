...
4. ~~**No client-side body yaw (`me_yaw`).**~~ **FIXED** (MOVE-FACE). The local body now predicts and
   ...
   load-bearing gap.)*
5. ~~**Item-use / ranged combat.**~~ **DONE.** Item-use beyond `P_EatItem` sends `P_ItemScript`
   (`packet_id::ITEM_SCRIPT=43`); ranged-weapon `MaxRange` now works — `ItemDef` retains `wtype`/`range`
   and the combat tick uses `effective_attack_range` (ranged → `range−0.5` when item-health > 0, else
   melee). Only the I_Image `WItemWindow` popup remains from this blocker. Talk paths covered (chat send exists).
