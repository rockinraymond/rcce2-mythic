...
| MENU-SCENE-b | Exact backdrop fidelity: dark-blue fogged void (`0,51,102`) + full-screen backdrop quad per screen (`Login.png`/`Character Selection.png`/`EULA.png`) + optional `Set.b3d` diorama at `(-210,-35,-145)` scale 30 + dolly-to-head on selection (currently the zone sky shows as the backdrop and the camera is a turntable, not a dolly) | PARTIAL — **2D-screen backdrops DONE ✅** | The pure-2D menu screens now draw their dedicated full-screen backdrop art instead of the zone sky: **EULA → `EULA.PNG`**, Options/Controls → `Login.PNG` (`assets::menu_backdrop_path`, case-insensitive `.PNG`/`.png`; lazily registered + drawn full-screen behind the now-semi-transparent panel). ref `MainMenu.bb:441-453,2932` | **`RCCE_SHOT` EULA PNG read 2026-06-02 — the `EULA.PNG` "LICENSE AGREEMENT" backdrop art renders behind the license text** (vs. the previous orbiting sky). Remaining: backdrop behind the **Login/CharSelect 3D char scene** (backdrop-behind-3D), the `Set.b3d` diorama, and dolly-to-head |
