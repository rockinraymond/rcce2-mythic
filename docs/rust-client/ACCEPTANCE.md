...
| MENU-SCENE-b | Exact backdrop fidelity: dark-blue fogged void (`0,51,102`) + full-screen backdrop quad per screen (`Login.png`/`Character Selection.png`/`EULA.png`) + optional `Set.b3d` diorama at `(-210,-35,-145)` scale 30 + dolly-to-head on selection (currently the zone sky shows as the backdrop and the camera is a turntable, not a dolly) | PARTIAL — **EULA + Login + Options/Controls backdrops DONE ✅** | These screens draw their dedicated full-screen backdrop art instead of the zone sky: **EULA → `EULA.PNG`, Login → `Login.PNG`**, Options/Controls → `Login.PNG` (`assets::menu_backdrop_path`, case-insensitive; lazily registered + drawn full-screen behind the panel). **Login works because it has no 3D character** (only CharSelect does). ref `MainMenu.bb:441-453,2932` | **`RCCE_SHOT` PNGs read 2026-06-02 — EULA shows the `EULA.PNG` "LICENSE AGREEMENT" art; Login shows the `Login.PNG` "Account Login" art** (both vs. the previous orbiting sky). Remaining: backdrop behind the **CharSelect 3D char scene**, the `Set.b3d` diorama, and dolly-to-head |
