...
| MENU-OPT | Options menu (Sound / Control / Other): adjustable settings reachable from the menu | PARTIAL — **Sound DONE ✅** | `Mode::Options` reached from Login with **F1**: a "Sound Options" panel with a master-volume bar + numeric %, a Mute toggle, Left/Right (or −/=) to adjust and `M` to mute — wired straight to `audio` (`set_master_volume`/`toggle_mute`, the same engine the in-world `[`/`]`/`M` keys use); Esc returns to Login. ref `MainMenu.bb` Sound options | unit test `volume_step_clamps` green; **`RCCE_OPTIONSTEST` PNG read 2026-06-02 — "Sound Options" panel, volume bar at 90%, "Mute: off"**. Control (keybind) + Graphics options still TODO |
