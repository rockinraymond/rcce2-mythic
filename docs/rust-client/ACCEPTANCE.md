...
Counting concrete criteria (excluding DEFERRED): **DONE ≈ 38, PARTIAL ≈ 29, MISSING ≈ 18** (ANIM-1, MOVE-5, MENU-SCENE, project-fix, TGT-1/2/3/4/6 closed 2026-06-01, Phases 1-4c). The headline gaps that block "true drop-in" are the four from the live play-test plus their dependencies:
...
4. Targeting cluster: ✅ TGT-1/TGT-2 (select + highlight + panel) + ✅ TGT-3/TGT-4/TGT-6 (context menu Interact/Attack/Examine/Trade, Phase 4c) **DONE**; **TGT-5** (NPC dialog `P_Dialog` rendering) + **CBT-1** (attack-the-mob loop) — remain → Phases 4d-5.
