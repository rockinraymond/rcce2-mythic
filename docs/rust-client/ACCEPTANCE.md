...
| MENU-SCENE | **The dedicated 3D menu scene** (THE known gap): dark-blue fogged void (`CameraFogColor 0,51,102`, range 300-5200) + a full-screen backdrop quad per screen (`Login.png` / `Character Selection.png` / `EULA.png`) + optional `Set.b3d` diorama at world `(-210,-35,-145)` scale 30 + the **selected character's actor mesh** at world `(30, ground-adjusted, 100)` playing `Anim_Idle`, camera ~150u back (offset −40 X), dollying to the head/chest on selection. **NOT** a spectator orbit of the gameplay zone | MISSING | ref `MainMenu.bb:80-92,441-453,1563-1576,1706-1727,2020-2086`; current Rust `render_menu` (`client_window.rs:1856-1864`) orbits `self.center` of the loaded zone, 2D-only overlay, no char models | `RCCE_AUTOSUBMIT` + `RCCE_SHOT` (read PNG: must show backdrop + posed character, not terrain) |
...
| TGT-6 | Context **Examine** sends `P_Examine [2]RuntimeID`; **Trade** sends `P_Trade [2]RuntimeID` | PARTIAL | ref `Interface3D.bb:694,703`; Rust `X`=examine key, no Trade-via-menu | live |
