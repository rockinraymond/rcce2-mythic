...
- **Zone duplicate** — closes the deferred kind from the duplicate iteration. New `DuplicateAreaTemplate` in `ServerAreas.bb` copies every field of the focused Area: WeatherChance + Outdoors + WeatherLink + Entry/Exit scripts + all 150 triggers + 2000 waypoints + 100 portals + 1000 spawn points + PvP + Gravity. Skips runtime state (Instances[], FirstWater, WeatherLinkArea pointer). EntityFactory_DuplicateZone wraps it with `EntityFactory_UniqueZoneName` so the duplicate's name (and therefore its on-disk .dat filename) doesn't collide with the source.
