...
- **Save All (Ctrl+S) + save-on-exit prompt** — new `Modules/Loom/SaveAll.bb` dispatches `Composer::commitSaveForKind` for every dirty kind in one shortcut. Esc-exit-empty now intercepts when any kind is dirty and opens an `ExitPrompt` modal with Save All / Discard All / Cancel buttons; Esc inside the modal = Cancel (panic-press doesn't lose work). Without this a session with edits across four tabs needed four clicks, and Esc-exit silently discarded unsaved changes.
