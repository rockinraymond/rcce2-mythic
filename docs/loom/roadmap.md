...
- **Help (F1) + display-font adoption** — new `Modules/Loom/Help.bb` modal listing every keybinding + mouse interaction in a Global / Browser / Composer / Ribbon table. Adopted `LoomTheme_UseDisplay` for the LOOM brand mark + every modal title (Palette / Timeline / Recents / BrokenRefs / ExitPrompt / Help itself). Footer hint gains "F1 for shortcuts" so users can discover the cheat sheet without already knowing about it.
